...
export default (function() {
  ...
  function template() {
    ...
    !(current = sessionStorage.getStorage('currentPage'))
      ? (current = 1)
      : (current = parseInt(current));
    !(init = sessionStorage.getStorage('paginationInit'))
      ? (init = 1)
      : (init = parseInt(init));
    ...
    return `
      <div class="page_nav" id="paginationBox">
        <span id="paginationPrev" class="page_nav-prev" data-prev="${init -
          config.maxNavs}"></span>  
  ...
  function pageClick() {
    pagination.list.addEventListener(
      'click',
      function(e) {
        e.preventDefault();
  ...
  function nextClick() {
    pagination.next.addEventListener(
      ...
      function(e) {
        ...
        session.currentPage = pagination.list.querySelector(
          '.active'
        ).innerText;
        session.offset = offsetcalc(session.currentPage);
  ...
  function prevClick() {
    pagination.prev.addEventListener(
      ...
      function(e) {
        ...
        pagination.prev.dataset.prev =
          pagination.itens[0].innerText - config.maxNavs;
