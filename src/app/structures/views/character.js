...
export default (function() {
  ...
  function htmlView() {
    return Characters.sources.map(
      character => `
      <a onclick="window.history.back();" class="link">VOLTAR</a>    

      <section id="${character.id}" class="character">
        <div class="character-background" style="background-image:url(${
  character.thumbnail.path
  }.${character.thumbnail.extension})"></div>
    ...
    character.name
  }" height="450px" width="300" class="character-info-img"/>
  ...
  function addWikis(character) {
    ...
    return ` 
      <h3 class="section_head">Para conhecer mais sobre o ${
  character.name
  } acesse:</h3>
      ${character.urls
    .map(
  ...
  function addComics(character) {
    ...
    .map(
      item =>
        `<li data-comic-uri="${
          item.resourceURI
        }" class="character-collections-list-item link">${item.name}</li>`
    )
    .join('')}
      </ul>
    `;
